    ...
    def get(self):
        ...
        if user_apprentice is not None:
            ...
            h = MysqlSearch().get_more(
                f"SELECT e.id,e.nickname,e.reg_time,e.avatar,e.setreal,etr.levels FROM {MEMBERS_TABLE} as e \
                    LEFT JOIN {ET_MEMBER_RELATIONS} as etr on etr.member_id=e.id \
                        WHERE etr.parent_id={g.user_id} \
            ...
            user_apprentice['邀请总奖励'] = sum_amounts
            user_apprentice['当天邀请奖励'] = today_amounts
            ...
            now_time = datetime.now().strftime("%Y-%m-%d")
            today_member = MysqlSearch().get_more(f"SELECT reg_time,id FROM {MEMBERS_TABLE} WHERE id IN (SELECT member_id FROM {ET_MEMBER_RELATIONS} WHERE parent_id='{g.user_id}') AND to_days(reg_time) = to_days('{now_time}')")
            if today_member:
                for today in today_member:
                    if today['id'] != g.user_id:
