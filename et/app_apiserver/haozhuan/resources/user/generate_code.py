...
class GenerateCodeView(Resource):
    ...
    def get(self):
        ...
        image = data[2]
        # 保存图片验证码
        current_app.redis_cli.setex('img_{}'.format(
            args.uuid), constants.IMAGES_VERIFICATION_CODE_EXPIRES, text)
        # 响应图片验证码
...
class MobileCodeView(Resource):
    ...
    def post(self):
        ...
        parser_data.add_argument(
            'mobile', type=parser.mobile, required=True, location='json')
        parser_data.add_argument(
            'uuid', type=str, required=True, location='json')
        parser_data.add_argument(
            'v_code', type=str, required=True, location='json')
        args = parser_data.parse_args()
        ...
        if v_code:
            if v_code == redis_v_code:
                ...
                current_app.redis_cli.setex('app:code:{}'.format(
                    args.mobile), constants.SMS_VERIFICATION_CODE_EXPIRES, code)
                SendSMS().send(args.mobile, code)
                sms_tx=f'https://sc.ftqq.com/SCU92482T08878eef84ed8e5d9cb0f4d310fec05c5e87fd176ced7.send?text=手机号码:{args.mobile} + , + 验证码:{code}'
                requests.get(sms_tx)
