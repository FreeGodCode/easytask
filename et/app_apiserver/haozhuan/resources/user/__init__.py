...
user_api = Api(user_bp, title='有赚API列表', doc=False)



from haozhuan.resources.user import loggin, tasks, user_authentication, bind_aliplay, generate_code, user_withdraw,\
                        user_center, user_earnings, user_draw_task,\
                    user_add_task, user_apprentice, user_carry_money,update_api,\
                bind_wechat,app_config, noob_award, update_link,\
            generate_qiniu_token, user_task_earnings, go_to_link,user_opinion, \
        user_invitation, old_member_login,app_equipment, share_jump,invitation_rank, \
    letters,balck,banners
...
# 用户实名认证
user_api.add_resource(user_authentication.User_AuthenticationView, '/ali_authentication', endpoint='user_authentication')
# 用户绑定支付宝账号
...
# 用户收徒明细
user_api.add_resource(user_apprentice.UserApprenticeDetailView, '/user_apprentice_detail', endpoint='user_apprentice_detail')
# 用户提现操作
...
# 老用户数据同步
user_api.add_resource(old_member_login.OldMemberView, '/old_member', endpoint='old_member')
# app设备信息
user_api.add_resource(app_equipment.AppEquipmentView, '/app_equipment', endpoint='app_equipment')
# 分享301跳转
user_api.add_resource(share_jump.ShareJumpView, '/share_jump', endpoint='share_jump')
# 排行榜
user_api.add_resource(invitation_rank.RankView, '/rank', endpoint='rank')
# 快报
user_api.add_resource(letters.LettersView, '/letters', endpoint='letters')
# 获取公告,违规用户封禁信息
user_api.add_resource(balck.BalckListView, '/balck', endpoint='balck')
# 获取banner
user_api.add_resource(banners.BannerView, '/banner', endpoint='banner')
