...
def handle_getusers():
    ...
    user = db.session.query(EtMember.id, EtMember.nickname, EtMember.status, EtMember.m_class, \
        EtMember.realname,EtMember.mobile,EtMember.IIUV,EtMember.setreal).filter(EtMember.id == memberid).first()
...
@route(bp, '/edit_user', methods=["POST","OPTIONS"])
@login_required
def handle_edituser():
    """
    用户信息修改接口
...
@route(bp, '/ban_user', methods=["POST","OPTIONS"])
@login_required
def handle_banuser():
    """
    封禁用户
