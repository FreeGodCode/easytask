...
from app.celery import async_alipay_service
...
def get_rank_data():
    ...
    with db.app.app_context() as app_context:
        ...
        rank_sql = f'SELECT COUNT(mr.id) AS counts, m.id, m.IIUV, m.avatar, m.nickname, m.realname \
        FROM et_member_relations AS mr \
        LEFT JOIN et_members AS m on m.id= mr.parent_id \
        WHERE week(mr.create_time) = week(now()) AND m.id is not null \
        GROUP BY (mr.parent_id) ORDER BY counts DESC LIMIT 0 ,{limit_num}'
        ...
        if send_data:
            for index, item in enumerate(rank_data):
                ...
                update_sql = f'UPDATE `activity_rewards` SET member_id={item["id"]},invite_count={item["counts"]},avatar="{avatar}" \
                WHERE fake=0 AND rank_num = {index + 1}'
        ...
        def auto_handle_wd(app_context, wd_id: int, member_id: int, status: int = 2):
            with app_context:
                ...
                if verify_orders.pay_status == 3:
                    res.update(dict(code=ResponseCode.Success, data={}, msg=f'该单已发放无法再次提现'))
                    return res
                ...
                user = db.session.query(EtMember.id, EtMember.nickname, EtMember.status, EtMember.m_class,
                                        EtMember.realname, EtMember.mobile, EtMember.IIUV, EtMember.balance,
                                        EtMember.balance_version, EtMember.setreal, EtMember.alipayid).filter(
                    EtMember.id == member_id).first()

                if not user:
                    res.update(dict(code=ResponseCode.Success, data={}, msg=f'用户信息异常'))
                    return res
                ...
                db.session.query(EtMemberWithdrawal).filter(EtMemberWithdrawal.id == wd_id).update(update_dict_ready)
                res_data = dict()
                ret = ''
                try:
                    if user_info['status'] == 2:
                        ...
                        db.session.query(EtMember).filter(
                            EtMember.id == mid, EtMember.balance_version == user_info['balance_version']).update(
                            user_update_dict)

                    db.session.commit()

                    # verify pass go alipay
                    ret = async_alipay_service.delay(serial_number=wd_id,
                    ...
                    res.update(code=ResponseCode.Success, data={}, msg=f'用户提现订单审核失败，数据异常{e}')
                    return res
