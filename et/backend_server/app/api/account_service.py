...
@route(bp, '/reg', methods=["POST","OPTIONS"])
def handle_reg():
    """
    账号注册接口
    :return: json
...
@route(bp, '/edit_account', methods=["POST","OPTIONS"])
@login_required
...
@route(bp, '/refresh_token', methods=["GET"])
# @login_required
def refresh_token():
    """
...
@route(bp, '/channel_list', methods=["GET"])
...
@route(bp, '/channel_add', methods=["POST"])
