...
def login_required(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        token = request.headers.get("xtoken")
        if not token:
            res.update(code=ResponseCode.PleaseSignIn, msg=f"{request.headers}")
            return res.data

        auth = Auth()
        user_name = auth.identify(token)
        if not user_name:
            res.update(code=ResponseCode.PleaseSignIn, msg=f"{user_name}")
            return res.data
